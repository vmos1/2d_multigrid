...
void f_read_gaugeU_heatbath(MArr2D U, params p){
    // Read phases from file
    double re,im;
    double phase;
    int x,y,j,d1,d2;
    ...
    char fname[1024];
    // sprintf(fname,"Near-null_L%d_blk%d_ndof%d.txt",p.size[0],p.block_x,p.n_dof_scale);
    sprintf(fname,"gauge_config_files/phase%db3.dat",p.size[0]);
...
int main (int argc, char *argv[])
   ...
   // exit(1); 
    ...
    // Test 2: Iterate to lower levels and relax
    #if 0
    int ll=p.nlevels;
    cout<<ll<<endl;
    #endif  
    ...
    // Test 3: St
    #if 1
        int start_lvl=p.nlevels-1;
        // Do Multigrid 
        if(p.nlevels>0){
        // Go down: fine -> coarse
            for(lvl=start_lvl;lvl<start_lvl+1;lvl++){
                printf("\nGoing down from %d to %d",lvl,lvl+1);
            ...
            // come up: coarse -> fine
            for(lvl=start_lvl+1;lvl>start_lvl;lvl--){
                printf("\nGoing up from %d to %d",lvl,lvl-1);
                relax(D[lvl],phi[lvl],r[lvl], lvl, num_iters,p,gs_flag); // Perform Gauss-Seidel
        ...
        resmag=f_get_residue_mag(D[start_lvl],phi[start_lvl],r[start_lvl],start_lvl,p);
        //resmag=f_get_residue_mag(D[0],phi[0],r[0],0,p);
    #endif 
