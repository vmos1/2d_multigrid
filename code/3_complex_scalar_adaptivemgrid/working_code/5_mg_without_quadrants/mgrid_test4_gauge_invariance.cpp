...
void f_near_null(MArr1D phi_null, MArr2D D, int level, int quad, int num_iters, int gs_flag, params p){
    ...
    for(int x=0;x<L; x++) for(int y=0; y<L; y++) for(d2=0; d2 < nf; d2++) r_zero(x+y*L)(d2)=0.0;  
        ...
        for(d1=0; d1< nc; d1++){
            ...
            // for(int x=0;x<L; x++) for(int y=0; y<L; y++) phi_temp(x+y*L)=phi_temp(x+y*L).conjugate(); 
...
int main (int argc, char *argv[])
    ...
    // omega : (X)(d1,d2)
    VArr1D phase(p.size[0]*p.size[0]);// Phase angle at each site
    VArr1D omega(p.size[0]*p.size[0]);// Complex value at each site : exp(phase)
    for (int j = 0; j < L*L ; j++) {
        phase(j) = ColorVector(p.n_dof[0]);
        omega(j) = ColorVector(p.n_dof[0]);
    ...
    // f_init_gauge(phase, omega, p, gen, dist);
    ...
    // f_test_gauge_transform(U,omega, r, phi, phi_null, D,gen, dist, p); 
