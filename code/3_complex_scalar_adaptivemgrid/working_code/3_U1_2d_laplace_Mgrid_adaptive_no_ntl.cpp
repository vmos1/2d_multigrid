...
double f_get_residue_mag(MArr2D D, VArr1D phi, VArr1D b, int level, params p){
    ...
    // for(int d=0;d<p.n_dof[level];d++)
        for(int x=0; x<L; x++) {
            for(int y=0;y<L; y++) {
                // res=pow(res,2)+std::abs(rtemp(x+y*L)(d)); // sum of absolute values.
    ...
    for(i=0; i<num_iter; i++){
        for(x=0; x<L; x++)
            for(y=0; y<L; y++){
                ...
                if (phitemp(x+y*L).norm()>1e20){
                // if (level>0 && phitemp(x+y*L).norm()<1e-5){
                    printf("phi_temp is very large for iter %d for x %d and y %d\n",i,x,y);
                    cout<<D(x+y*L,0).inverse()<<endl;
                    cout<<D(x+y*L,0).rows()<<D(x+y*L,0).cols()<<endl;
                    cout<<D(x+y*L,0)<<endl;
                    cout<<D(x+y*L,1).rows()<<D(x+y*L,1).cols()<<endl;
                    cout<<D(x+y*L,1)<<endl;
                    cout<<D(x+y*L,3).rows()<<D(x+y*L,3).cols()<<endl;
                    cout<<D(x+y*L,3)<<endl;
                    
                    cout<<res(x+y*L)<<endl<<endl;
                    cout<<phitemp((x+1)%L+y*L)<<endl;
                    cout<<phitemp((x-1)%L+y*L)<<endl;
                    cout<<phitemp(x+((y+1)%L)*L)<<endl;
                    cout<<phitemp(x+((y-1)%L)*L)<<endl;
                    cout<<phitemp(x+y*L)<<endl;
                    exit(1);
                }
            // Gauss-Seidel
...
int main (int argc, char *argv[])
    ...
    // cout<<U(0,0).rows()<<U(0,0).cols()<<U(0,0)<<endl;
    // cout<<D[0](0,1).rows()<<D[0](0,1).cols()<<D[0](0,1)<<endl;
    // cout<<"Start phases"<<endl;
