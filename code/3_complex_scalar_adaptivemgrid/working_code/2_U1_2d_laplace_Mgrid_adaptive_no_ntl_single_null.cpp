...
void f_block_norm(Complex* vec, int L, int Lc){
    ...
    for(int x=0;x<Lc; x++) 
        for(int y=0; y<Lc; y++) {
            ...
            norm=sqrt(pow(abs(vec[xa+ya*L]),2) 
                     +pow(abs(vec[xa+yb*L]),2)
                     +pow(abs(vec[xb+ya*L]),2)
                     +pow(abs(vec[xb+yb*L]),2));
            // printf("Norm %f\n",norm);
...
void f_near_null(Complex* phi_null, Complex* D, int level, int quad, int num_iters, int gs_flag, params p){
    ...
    for (int i=0;i<num;i++){
        relax(D,phi_null,r_zero, level, 50,p,gs_flag); // relaxation by 50 each time, then normalize
    ...
    f_block_norm(phi_null,L,Lc);
    
    delete[] r_zero; 
}

void f_ortho(Complex *null_1, Complex*null_2, int level, params p) { 
    // Orthogonalize near-null vector w.r.t previous ones
    Complex dot;
    double norm;
    int xa,xb,ya,yb,x,y,L,Lc;
    ...
    for(int x=0;x<Lc; x++) 
        for(int y=0; y<Lc; y++) {
            ...
            norm=sqrt(pow(abs(null_1[xa+ya*L]),2) 
                     +pow(abs(null_1[xa+yb*L]),2)
                     +pow(abs(null_1[xb+ya*L]),2)
                     +pow(abs(null_1[xb+yb*L]),2));
            
            printf("Norm %.20f\n",norm);
            dot=( conj(null_2[xa+ya*L])*null_1[xa+ya*L]
                + conj(null_2[xa+yb*L])*null_1[xa+yb*L]
                + conj(null_2[xb+ya*L])*null_1[xb+ya*L]
                + conj(null_2[xb+yb*L])*null_1[xb+yb*L]);
            
            // Can avoid dividing by norm, since it is 1.
            null_2[xa+ya*L]+= -((dot/norm)*(null_1[xa+ya*L])); 
            null_2[xa+yb*L]+= -((dot/norm)*(null_1[xa+yb*L])); 
            null_2[xb+ya*L]+= -((dot/norm)*(null_1[xb+ya*L])); 
            null_2[xb+yb*L]+= -((dot/norm)*(null_1[xb+yb*L])); 
        }
    f_block_norm(null_2,L,Lc);
}
...
void f_test3_hermiticity(Complex *D, int level, params p){
...
void f_test4_hermiticity_full(Complex *vec, Complex *D,int level, params p, int quad){
    // Test if all D's are Hermitian
    Complex a1;
    int Lf,x,y;
    double Epsilon=1.0e-12;
...
int main (int argc, char *argv[])
    ...
    for(int i=0; i<p.nlevels+1; i++){
        ...
        phi_null2[i]=new Complex [p.size[i]*p.size[i]];
        // phi_null2[i]=new Complex [p.size[i]*p.size[i]][num_null];
        
        
        phi_null_new[i]=new Complex* [num_null];
        for(int j=0; j<num_null; j++){
            phi_null_new[i][j]=new Complex [p.size[i]*p.size[i]];}
        }

    for(int i=0; i< p.nlevels+1; i++){
        for(int j=0; j< num_null; j++){
            for(int k=0; k< p.size[i]*p.size[i]; k++){
                phi_null_new[i][j][k]=0.0;
        }}}
    ...
    for(int i=0; i< p.nlevels+1; i++){
        for(int j=0; j< p.size[i]*p.size[i]; j++){
            ...
            phi_null2[i][j]=dist(gen); // Random initialization
        }}   
    
    Complex *U; // Link fields at each point with two directions
    U=new Complex [p.size[0]*p.size[0]*2]; 
    ...
    for(int j=0; j< p.size[0]*p.size[0]*2; j++){
        U[j]=1.0; 
        // U[j]=std::polar(1.0,PI);// Global phase of -1
        // U[j]=rnd1; // Random global phase 
        // U[j]=std::polar(1.0,dist(gen)); // Random local phase
    }
   ...
   //      for(int y=0; y<L; y++){ 
   //          for(int k=0; k<2; k++){
   //              fprintf(pfile4,"%f+i%f\n",real(U[x+L*y+k*L*L]),imag(U[x+L*y+k*L*L]));}}}
   //  fclose(pfile4);
    ...
    for(int x=0; x<L; x++){ 
        for(int y=0; y<L; y++){ 
            ...
            U[x+L*y+k*L*L]=complex<double>(re,im);
            }}}
    fclose(pfile5);
