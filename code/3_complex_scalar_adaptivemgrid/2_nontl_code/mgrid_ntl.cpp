        ...
        // Go down: fine -> coarse
            for(lvl=0;lvl<p.nlevels;lvl++){
                ...
                if((lvl==p.nlevels-1)&&(t_flag==1)){// non-telescoping only for going to the lowest level
                    for(int i=0;i<n_copies;i++){ // Project 4 independent ways
                        f_restriction_res(r_tel[i],r[lvl],phi[lvl],D[lvl],phi_null[lvl], lvl,p,i+1); } 
                }
            ...
            for(lvl=p.nlevels;lvl>=0;lvl--){
                ...
                if((lvl==p.nlevels)&&(t_flag==1)){// non-telescoping only for coming up from the lowest level
                    ...
                    for (int i=0; i<n_copies; i++) for (int j=0; j<j_size*j_size ; j++) for(int d1=0; d1<j_ndof; d1++) phi_tel[i](j)(d1) = 0.0;

                    for(int i=0;i<n_copies;i++){ // Project 4 independent ways
                        relax(D[lvl],phi_tel[i],r_tel[i], lvl, num_iters,p,gs_flag); // Relaxation
                        if(lvl>0) f_prolongate_phi(phi[lvl-1],phi_tel[i], phi_null[lvl-1], lvl,p,i+1);
                    }
                    // Average over values 
