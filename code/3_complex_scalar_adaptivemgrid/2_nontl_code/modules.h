...
void f_compute_coarse_matrix(MArr2D Dc, MArr2D Df, MArr1D phi_null, int level, int quad, params p){
    // Compute D matrix for lower level
    // Given a lvl, use D[lvl] (Df) and phi_null[lvl] to compute D[lvl+1] (Dc)
    // D_c = P D_f P^dagger
    ...
    */
    ...
    for(xc=0; xc<Lc; xc++) for(yc=0; yc<Lc; yc++) {
        ...
        if (quad==1)      {xbase=p.block_x * xc;               ybase=p.block_y * yc; }
        else if (quad==2) {xbase=(p.block_x * xc -1 +Lf )%Lf;  ybase=p.block_y * yc; }
        else if (quad==3) {xbase=(p.block_x * xc -1 +Lf )%Lf;  ybase=(p.block_y * yc - 1 +Lf)%Lf; }
        else if (quad==4) {xbase=p.block_x * xc;               ybase=(p.block_y * yc - 1 +Lf)%Lf; }
...
//             Dc(x+y*Lc,0)  =( phi_null(xa+ya*Lf) * Df(xa+ya*Lf,0)* phi_null(xa+ya*Lf).adjoint()
//                                    + phi_null(xa+yb*Lf) * Df(xa+yb*Lf,0)* phi_null(xa+yb*Lf).adjoint()
//                                    + phi_null(xb+ya*Lf) * Df(xb+ya*Lf,0)* phi_null(xb+ya*Lf).adjoint()
//                                    + phi_null(xb+yb*Lf) * Df(xb+yb*Lf,0)* phi_null(xb+yb*Lf).adjoint()
                                            
//                                    + phi_null(xa+ya*Lf) * Df(xa+ya*Lf,1)* phi_null(xb+ya*Lf).adjoint()
//                                    + phi_null(xb+ya*Lf) * Df(xb+ya*Lf,2)* phi_null(xa+ya*Lf).adjoint()
//                                    + phi_null(xb+ya*Lf) * Df(xb+ya*Lf,3)* phi_null(xb+yb*Lf).adjoint()
//                                    + phi_null(xb+yb*Lf) * Df(xb+yb*Lf,4)* phi_null(xb+ya*Lf).adjoint()
//                                    + phi_null(xb+yb*Lf) * Df(xb+yb*Lf,2)* phi_null(xa+yb*Lf).adjoint()
//                                    + phi_null(xa+yb*Lf) * Df(xa+yb*Lf,1)* phi_null(xb+yb*Lf).adjoint()
//                                    + phi_null(xa+yb*Lf) * Df(xa+yb*Lf,4)* phi_null(xa+ya*Lf).adjoint()
//                                    + phi_null(xa+ya*Lf) * Df(xa+ya*Lf,3)* phi_null(xa+yb*Lf).adjoint() );

            // // x+1 term: fixed xb -> xb+1
            // Dc(x+y*Lc,1)=  ( phi_null(xb+ya*Lf) * Df(xb+ya*Lf,1)*phi_null((xb+1)%Lf+ya*Lf).adjoint()
            //                        + phi_null(xb+yb*Lf) * Df(xb+yb*Lf,1)*phi_null((xb+1)%Lf+yb*Lf).adjoint());
            // // x-1 term: fixed xa -> xa-1
            // Dc(x+y*Lc,2)=  ( phi_null(xa+ya*Lf) * Df(xa+ya*Lf,2)*phi_null((xa-1+Lf)%Lf+ya*Lf).adjoint()
            //                        + phi_null(xa+yb*Lf) * Df(xa+yb*Lf,2)*phi_null((xa-1+Lf)%Lf+yb*Lf).adjoint());
            // // y+1 term: fixed yb -> yb+1
            // Dc(x+y*Lc,3)=  ( phi_null(xa+yb*Lf) * Df(xa+yb*Lf,3)*phi_null(xa+((yb+1)%Lf)*Lf).adjoint()
            //                        + phi_null(xb+yb*Lf) * Df(xb+yb*Lf,3)*phi_null(xb+((yb+1)%Lf)*Lf).adjoint());
            // // y-1 term: fixed ya -> ya-1
            // Dc(x+y*Lc,4)=  ( phi_null(xa+ya*Lf) * Df(xa+ya*Lf,4)*phi_null(xa+((ya-1+Lf)%Lf)*Lf).adjoint()
            //                        + phi_null(xb+ya*Lf) * Df(xb+ya*Lf,4)*phi_null(xb+((ya-1+Lf)%Lf)*Lf).adjoint());
       }
}
