...
int main (int argc, char *argv[])
    ...
    p.scale[0]=1.0/(2.0+m_square*p.a[0]*p.a[0]);// 1/(4+m^2 a^2) 
    p.n_dof[0]=2;
    p.n_dof_scale=4; // N_dof at higher levels
    p.block_x=block_x;
    p.block_y=block_y;
    p.n_color=1;
    p.spinor_dim=2;
    ...
    for(int i=0; i< p.size[0]*p.size[0]; i++)
        for(int j=0; j< 2; j++){
            U(i,j) = ColorMatrix(p.n_color,p.n_color);
            // Initialize
    ...
    // Arrays for next-to-lowest level
    VArr1D phi_tel_f[4],r_tel_f[4]; // finer levels
    MArr2D D_tel[4];
    MArr1D phi_null_tel[4];
    
    if (t_flag!=0 & p.nlevels>0){
        j_size=p.size[p.nlevels-1];
        j_ndof=p.n_dof[p.nlevels-1]; // dof in next-to-lowest levels

        for (int q_copy=0; q_copy<4; q_copy++){
            phi_tel_f[q_copy]=VArr1D(j_size*j_size);
            r_tel_f[q_copy]=VArr1D(j_size*j_size);
            for (int j = 0; j < j_size*j_size ; j++){
                phi_tel_f[q_copy](j) = ColorVector(j_ndof);
                r_tel_f[q_copy](j) = ColorVector(j_ndof);
                // Initialize
                for(int d1=0;d1<j_ndof;d1++){
                    phi_tel_f[q_copy](j)(d1) = 0.0;
                    r_tel_f[q_copy](j)(d1)=0.0;
                }}}
        ...
        // Go down: fine -> coarse
            ...
            for(lvl=p.nlevels;lvl>=0;lvl--){
                if((lvl==p.nlevels)&&(t_flag==1)){// non-telescoping only for coming up from the lowest level
                    ...
                    f_min_res(a_copy, phi_tel_f, D[lvl-1], r[lvl-1], phi[lvl-1], n_copies, lvl-1, p);   // Min res
                    cout<<endl;
